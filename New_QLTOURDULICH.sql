...
(
	...
	FILENAME = 'D:\QL_TourDuLich_Primary.mdf',
	SIZE = 10MB,
	MAXSIZE = 100MB,
	FILEGROWTH = 10%
)
...
(
	...
	FILENAME = 'D:\QL_TourDuLich_Log.ldf',
	SIZE = 10MB,
	MAXSIZE = 100MB,
	FILEGROWTH = 10%
)
...
AS
	BEGIN
		declare @matour_in varchar(10), @matour_del varchar(10), @slb int, @slc int
		select @matour_in = (select inserted.MaTour from inserted group by inserted.MaTour),
				@matour_del = (select deleted.MaTour from deleted group by deleted.MaTour)
		if (select count(*) from inserted) > 0
			begin
				Update tour
				set SoLuongBook = SoLuongBook + 1
				where tour.MaTour = @matour_in
				...
				Update tour
				set TongTien = SoLuongBook * ChiPhiDauNguoi
				where tour.MaTour = @matour_in
			end
		if (select count(*) from deleted) > 0
			begin
				set @slb = (select count(*) from ve where ve.MaTour = @matour_del)

				Update tour
				set SoLuongCon = 20 - @slb
				where tour.MaTour = @matour_del

				Update tour
				set SoLuongBook = (select count(*) from ve where ve.MaTour = @matour_del)
				where tour.MaTour = @matour_del
		...
		--Update tour
		--set SoLuongBook = @soluongbook, SoLuongCon = SoLuongCon - @soluongbook, TongTien = SoLuongBook * ChiPhiDauNguoi
...
GO
INSERT INTO VE(NgayBook, MaTour, MaKH)
VALUES
(getdate(),'T001','KH0001')
GO
INSERT INTO VE(NgayBook, MaTour, MaKH)
